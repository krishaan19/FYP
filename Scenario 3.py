...
total_wind_capacity_mw = 225  # Total capacity of wind farm

v_cut_in = 3.0    # Minimum operational wind speed (m/s)
v_rated = 12.0    # Rated wind speed (m/s)
v_cut_out = 25.0  # Shutdown wind speed (m/s)

# Wind power availability function (simple power curve)
...
# Compute available wind power for each hour
...
A_s = 1860000             # Total PV area in m²
cost_conv = 80            # Placeholder for conventional cost per MWh (unused)
emission_conv = 0.5       # Placeholder for emission rate (unused)
battery_capacity = 100    # Battery capacity in MWh
battery_charge_rate = 25  # Max charge/discharge rate in MW
battery_eff = 0.95        # Battery round-trip efficiency

ev_capacity = 201.25      # EV fleet energy capacity in MWh
ev_charge_rate = 30       # EV charging/discharging rate in MW
ev_eff = 0.90             # EV round-trip efficiency
ev_initial_soc = 100      # Initial state-of-charge (MWh)
ev_min_soc = 40           # Minimum required SoC when EVs are away
...
daily_ev_driving_energy = 3.0  # Total daily driving consumption (MWh)
driving_hours = [t for t in T if ev_available[t] == 0]  # Hours when EVs are away

# Driving energy consumption per hour (spread evenly across driving hours)
...
# EV export to grid cannot exceed actual discharging
...
# Total shifted load must equal total shiftable load
model.shift_balance = pyo.Constraint(
    expr=sum(model.D_shift[t] for t in model.T) == sum(shiftable_load)
)

# Limit max shifted load per hour
max_shiftable_per_hour = 1.5 * np.max(shiftable_load)  # Conservative hourly cap
model.shift_limit = pyo.Constraint(
    model.T,
    rule=lambda m, t: m.D_shift[t] <= max_shiftable_per_hour
)

# Ramp Up Constraint: penalize fast increase in shifted load
...
# Ramp Down Constraint: penalize fast decrease
...
# Solar power limited by selected tilt angle irradiance
def solar_output_rule(model, t):
    return model.P_solar[t] <= (eta * A_s / 1_000_000) * sum(
        R_S[(t, a)] * model.use_angle[t, a] for a in model.A)
model.solar_output = pyo.Constraint(model.T, rule=solar_output_rule)

# Only one tilt angle can be used per hour
...
# Wind power cannot exceed what is available from resource
model.wind_limit = pyo.Constraint(model.T, rule=lambda m, t: m.P_wind[t] <= P_wind_avail[t])

# Power balance at every time step (supply = demand)
...
# Battery SoC evolution over time
...
# EV charge/discharge power limits
...
# Mode-based EV control: charge only if in charging mode
...
# EV SoC dynamics including driving consumption
...
# Minimum SoC requirement when EVs are away from grid
